...
// =============================================================================
// ONGLET IP Calculateur
// =============================================================================

function setupIpCalcTab() {
  // Remplir le select CIDR /1 → /32
  ...
  function card(id, label, value, color) {
    document.getElementById(id).innerHTML = `
      ...
      <div style="font-size:18px;font-weight:700;color:${color || 'var(--tx-1)'};font-family:'JetBrains Mono','Courier New',monospace;display:flex;align-items:center;justify-content:space-between;gap:8px">
        ...
        <button onclick="navigator.clipboard.writeText('${value}').then(()=>showToast('Copié','success'))" style="background:none;border:none;cursor:pointer;color:var(--tx-3);padding:2px 4px;border-radius:4px;font-size:11px;flex-shrink:0" title="Copier">
